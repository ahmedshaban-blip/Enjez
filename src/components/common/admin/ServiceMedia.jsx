...
export default function ServiceMedia({
  existingImages = [], // array of {url, path|null}
  ...
  return (
    <div className="rounded-xl border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900/50 p-6">
      ...
      >
        ...
        <p className="text-xs text-gray-500">JPG, PNG, GIF (max 5MB per image)</p>
        ...
        <>
          ...
          <div className="flex flex-wrap gap-4 mb-4">
            {existingImages.map((img, idx) => {
              const url = img?.url ?? "";
              return (
                <div key={url} className="relative border rounded-lg p-2 bg-gray-50 dark:bg-gray-800">
                  <img src={url} alt={url} className="h-24 w-24 object-cover rounded-md" />
                  <button
                    type="button"
                    onClick={() => onDeleteExistingImage(idx)}
        ...
        <>
          ...
          <div className="flex flex-wrap gap-4">
            {selectedFiles.map((file, idx) => (
              <div key={idx} className="relative border rounded-lg p-2 bg-gray-50 dark:bg-gray-800">
                <img src={URL.createObjectURL(file)} alt={file.name} className="h-24 w-24 object-cover rounded-md" />
                <button
                  type="button"
                  onClick={() => onDeleteNewFile(idx)}
                  className="absolute -top-2 -right-2 bg-red-500 text-white rounded-full p-1 hover:bg-red-600"
                >
                  <span className="material-symbols-outlined text-sm">close</span>
                </button>
