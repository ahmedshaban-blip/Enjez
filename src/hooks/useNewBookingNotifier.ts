    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      snapshot.docChanges().forEach(async (change) => {
        ...
        const [userSnap, serviceSnap, agentSnap] = await Promise.all([
          ...
          booking.serviceId
            ? getDoc(doc(db, "services", booking.serviceId))
            : Promise.resolve(null),
          booking.agentId
            ? getDoc(doc(db, "agents", booking.agentId))
            : Promise.resolve(null),
        ]);
        ...
        const userName =
          userSnap && userSnap.exists()
            ...
            : "";
        const serviceName =
          serviceSnap && serviceSnap.exists()
            ? (serviceSnap.data() as any).name || ""
            : "";
        const agentName =
          agentSnap && agentSnap.exists()
            ? (agentSnap.data() as any).name || ""
            : "";
        ...
        const notifRef = doc(db, "notifications", bookingId);
        const existingNotif = await getDoc(notifRef);

        if (existingNotif.exists()) {
          console.log("====================================");
          console.log(
            "Notification already exists (maybe adminSaw already true)"
          );
          console.log("====================================");
